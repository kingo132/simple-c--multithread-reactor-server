...
void TcpHandler::accept_client(int server_fd, ClientManager& client_manager, EventDispatcher* dispatcher, dll_func_t* dll_functions, size_t recv_buffer_size, size_t send_buffer_size) {
    ...
    if (client_fd >= 0) {
        ...
        if (dll_functions->handle_open && dll_functions->handle_open(&ci->send_buffer, &(send_len), &ci->socket_info) < 0) {
            LOG_TRACE("handle_open error, remove client.");
...
ssize_t TcpHandler::receive_data(ClientInfo& client, dll_func_t* dll_functions, RingQueue& recv_queue) {
    ...
    if (bytes_received > 0) {
        ...
        int result;
        while ((result = dll_functions->handle_input(client.recv_buffer, (int)client.recv_len, &client.socket_info)) > 0) {
        ...
        // If `handle_input` returns 0, it means we're still waiting for more data.
        if (result == 0) {
            return 0;
        } else {
            // `handle_input` returned a negative value, indicating an error.
